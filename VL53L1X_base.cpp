...
void VL53L1X_BASE::setAddress(uint8_t new_addr)
{
...
// mode.
bool VL53L1X_BASE::init(bool io_2v8)
{
...
void VL53L1X_BASE::SetROICenter(uint8_t ROICenter)
{
	writeReg(VL53L1_ROI_CONFIG__USER_ROI_CENTRE_SPAD, ROICenter);
}

void VL53L1X_BASE::GetROICenter(uint8_t *ROICenter)
{
	uint8_t tmp;
	tmp = readReg16Bit(VL53L1_ROI_CONFIG__USER_ROI_CENTRE_SPAD);
	*ROICenter = tmp;
}

void VL53L1X_BASE::SetROI(uint16_t X, uint16_t Y)
{
	uint8_t OpticalCenter;

	OpticalCenter = readReg16Bit(VL53L1_ROI_CONFIG__MODE_ROI_CENTRE_SPAD);
	if (X > 16)
		X = 16;
	if (Y > 16)
		Y = 16;
	...
	writeReg(VL53L1_ROI_CONFIG__USER_ROI_CENTRE_SPAD, OpticalCenter);
	writeReg(VL53L1_ROI_CONFIG__USER_ROI_REQUESTED_GLOBAL_XY_SIZE,
		       (Y - 1) << 4 | (X - 1));
}

void VL53L1X_BASE::GetROI_XY(uint16_t *ROI_X, uint16_t *ROI_Y)
{
	uint8_t tmp;

	tmp = readReg16Bit(VL53L1_ROI_CONFIG__USER_ROI_REQUESTED_GLOBAL_XY_SIZE);
	*ROI_X = ((uint16_t)tmp & 0x0F) + 1;
	*ROI_Y = (((uint16_t)tmp & 0xF0) >> 4) + 1;
}

void VL53L1X_BASE::ClearInterrupt()
{
	writeReg(VL53L1_SYSTEM__INTERRUPT_CLEAR, 0x01);
}

// Write an 8-bit register
void VL53L1X_BASE::writeReg(uint16_t reg, uint8_t value)
{
...
void VL53L1X_BASE::writeReg16Bit(uint16_t reg, uint16_t value)
{
  Wire.beginTransmission(address);
  Wire.write((reg >> 8) & 0xFF); // reg high byte
  Wire.write( reg       & 0xFF); // reg low byte
...
void VL53L1X_BASE::writeReg32Bit(uint16_t reg, uint32_t value)
{
  Wire.beginTransmission(address);
  Wire.write((reg >> 8) & 0xFF); // reg high byte
  Wire.write( reg       & 0xFF); // reg low byte
...
uint8_t VL53L1X_BASE::readReg(regAddr reg)
{
  uint8_t value;
...
uint16_t VL53L1X_BASE::readReg16Bit(uint16_t reg)
{
  uint16_t value;
...
uint32_t VL53L1X_BASE::readReg32Bit(uint16_t reg)
{
  uint32_t value;
...
bool VL53L1X_BASE::setDistanceMode(DistanceMode mode)
{
...
bool VL53L1X_BASE::setMeasurementTimingBudget(uint32_t budget_us)
{
...
// based on VL53L1_SetMeasurementTimingBudgetMicroSeconds()
uint32_t VL53L1X_BASE::getMeasurementTimingBudget()
{
...
void VL53L1X_BASE::startContinuous(uint32_t period_ms)
{
...
void VL53L1X_BASE::stopContinuous()
{
...
// measurement)
uint16_t VL53L1X_BASE::read(bool blocking)
{
  if (blocking)
  {
    startTimeout();
...
uint16_t VL53L1X_BASE::readSingle(bool blocking)
{
  writeReg(SYSTEM__INTERRUPT_CLEAR, 0x01); // sys_interrupt_clear_range
...
const char * VL53L1X_BASE::rangeStatusToString(RangeStatus status)
{
  switch (status)
  {
    case RangeValid:
...
// timeoutOccurred()?
bool VL53L1X_BASE::timeoutOccurred()
{
...
void VL53L1X_BASE::setupManualCalibration()
{
...
void VL53L1X_BASE::getRangingData()
{
...
uint32_t VL53L1X_BASE::decodeTimeout(uint16_t reg_val)
{
...
uint16_t VL53L1X_BASE::encodeTimeout(uint32_t timeout_mclks)
{
...
uint32_t VL53L1X_BASE::timeoutMclksToMicroseconds(uint32_t timeout_mclks, uint32_t macro_period_us)
{
...
uint32_t VL53L1X_BASE::timeoutMicrosecondsToMclks(uint32_t timeout_us, uint32_t macro_period_us)
{
...
uint32_t VL53L1X_BASE::calcMacroPeriod(uint8_t vcsel_period)
{
